...
def expected_source_reference_paths(
    ...
    paths = []
    for idx in indices:
        rel_path = Path(dataset.samples[idx]).resolve().relative_to(src_root.resolve())
...
def main():
    ...
    source_dataset = TranslateDataset(
    ...
    if is_main:
        ...
        if can_reuse:
            ...
            for idx, ref_path in zip(source_indices, source_ref_paths):
                src_path = Path(source_dataset.samples[idx])
