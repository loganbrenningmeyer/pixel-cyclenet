...
def main():
    ...
    # -------------------------
    rgb_parent_dirs = as_parent_dir_set(cfg_select(translate_config, "data.rgb_parent_dirs", None))
    if rgb_parent_dirs is None:
        raise ValueError(
            "data.rgb_parent_dirs must be set for the segmentation sweep so RGB files can be matched to label masks."
        )

    label_parent_dir = str(
        cfg_select(
            translate_config,
            "data.label_parent_dir",
            cfg_select(cyclenet_config, "data.label_parent_dir", "gt_ss_mask"),
        )
    )
    num_seg_classes = int(
        cfg_select(translate_config, "model.num_seg_classes", cyclenet_config.model.num_seg_classes)
    )
    ...
    if len(dataset) == 0:
        raise RuntimeError(f"No source images found under {translate_config.data.src_dir}.")
        ...
        for noise_strength in sweep_strengths:
            for cfg_weight in sweep_cfgs:
                ...
                if is_main:
                    ...
                    print(f"\n[{combo}] writing to {out_dir}", flush=True)
                ...
                with torch.inference_mode():
                    ...
                    if is_main:
                        loader_iter = tqdm(dataloader, desc=f"Translating {combo}", unit="batch")
