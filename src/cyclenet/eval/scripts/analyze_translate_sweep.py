...
from cyclenet.eval.embed import CLIPEmbedder, DeepLabEmbedder, InceptionEmbedder
from cyclenet.eval.plotting.project import (
    plot_proj_density,
    plot_proj_density_marginal,
    plot_proj_scatter,
)
from cyclenet.eval.project import PcaProjector, UmapProjector
...
def cfg_select(config: Any, key: str, default: Any = None) -> Any:
...
def discover_image_paths(root_dir: Path, prefer_parent_name: str | None = "opt") -> list[Path]:
    ...
    if prefer_parent_name is None:
        return all_paths

    preferred = [path for path in all_paths if path.parent.name == prefer_parent_name]
...
def build_embedder(config: Any) -> Any:
    model_name = str(config.embedding.model).lower()
    model_path = cfg_select(config, "embedding.model_path")

    if model_name == "clip":
        return CLIPEmbedder(clip_path=model_path)
    if model_name == "inception":
        return InceptionEmbedder()
    ...
    raise ValueError(f"Unsupported embedding model: {config.embedding.model}")


def fit_reference_projectors(
    projection_config: Any,
    cache_dir: Path,
) -> tuple[dict[str, tuple[np.ndarray, np.ndarray]], list[Path], list[Path]]:
    cache_dir.mkdir(parents=True, exist_ok=True)

    seed = int(cfg_select(projection_config, "run.seed", 42))
    batch_size = int(cfg_select(projection_config, "embedding.batch_size", 64))
    num_samples = int(cfg_select(projection_config, "embedding.num_samples", 1000))
    random_state = int(cfg_select(projection_config, "projection.random_state", 42))
    n_components = int(cfg_select(projection_config, "projection.n_components", 2))

    embedder = build_embedder(projection_config)

    sim_paths = sample_paths(
        discover_image_paths(Path(projection_config.data.sim_dir), prefer_parent_name="opt"),
        num_samples,
        seed,
    )
    real_paths = sample_paths(
        discover_image_paths(Path(projection_config.data.real_dir), prefer_parent_name="opt"),
        num_samples,
        seed,
    )

    sim_emb = load_or_compute_embeddings(embedder, sim_paths, batch_size, cache_dir / "sim_embed.npy")
    real_emb = load_or_compute_embeddings(embedder, real_paths, batch_size, cache_dir / "real_embed.npy")

    ref_emb = np.concatenate([sim_emb, real_emb], axis=0)
    projectors = {
        "umap": UmapProjector(n_components=n_components, random_state=random_state),
        "pca": PcaProjector(n_components=n_components, random_state=random_state),
    }

    ref_coords: dict[str, tuple[np.ndarray, np.ndarray]] = {}
    for name, projector in projectors.items():
        coords = projector.fit(ref_emb)
        sim_coords = coords[: len(sim_emb)]
        real_coords = coords[len(sim_emb) :]
        ref_coords[name] = (sim_coords, real_coords)

    return (
        {
            name: (projector, *ref_coords[name])
            for name, projector in projectors.items()
        },
        sim_paths,
        real_paths,
...
def plot_projection_figure(
    ...
    show_points = bool(cfg_select(plotting_config, "points.show", True))
    show_density = bool(cfg_select(plotting_config, "density.show", True))
    show_marginal = bool(cfg_select(plotting_config, "marginal.show", False))
    ...
    common_kwargs = dict(
        ...
        ylabel=ylabel,
        save_path=save_path,
        max_points_per_group=cfg_select(plotting_config, "points.max_points_per_group", 2000),
        seed=seed,
    )

    if show_marginal and show_density:
        plot_proj_density_marginal(
            **common_kwargs,
            fill=bool(cfg_select(plotting_config, "density.fill", False)),
            alpha=float(cfg_select(plotting_config, "density.alpha", 0.35)),
            point_alpha=float(cfg_select(plotting_config, "points.alpha", 0.1)),
            point_size=float(cfg_select(plotting_config, "points.size", 10.0)),
        ...
        plot_proj_scatter(
            **common_kwargs,
            alpha=float(cfg_select(plotting_config, "points.alpha", 0.1)),
            point_size=float(cfg_select(plotting_config, "points.size", 10.0)),
        )
    else:
        raise ValueError("At least one of plotting.points.show or plotting.density.show must be True.")


def unique_projection_rows(ranked: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    seen: set[str] = set()
    ...
    for row in ranked:
        ...
        if checkpoint and checkpoint not in best_by_checkpoint:
            best_by_checkpoint[checkpoint] = row

    for row in best_by_checkpoint.values():
        out_dir = str(row.get("out_dir", ""))
        if out_dir and out_dir not in seen:
            rows.append(row)
            seen.add(out_dir)
...
def plot_candidate_projections(
    ...
    if not projection_config_path.exists():
        print(f"Projection config not found, skipping embedding plots: {projection_config_path}")
        return

    projection_config = OmegaConf.load(projection_config_path)
    seed = int(cfg_select(projection_config, "run.seed", 42))
    batch_size = int(cfg_select(projection_config, "embedding.batch_size", 64))
    num_samples = int(cfg_select(projection_config, "embedding.num_samples", 1000))

    projections_dir = out_dir / "projections"
    cache_dir = cache_root if cache_root is not None else projections_dir / "cache"
    cache_dir.mkdir(parents=True, exist_ok=True)

    projector_bundle, _sim_paths, _real_paths = fit_reference_projectors(projection_config, cache_dir / "reference")
    embedder = build_embedder(projection_config)

    plotting_config = projection_config.plotting
    labels = [
        str(cfg_select(plotting_config, "labels.sim", "Sim")),
        str(cfg_select(plotting_config, "labels.real", "Real")),
        str(cfg_select(plotting_config, "labels.translated", "Translated")),
    ]
    colors = [
        str(cfg_select(plotting_config, "colors.sim", "#6b7280")),
        str(cfg_select(plotting_config, "colors.real", "#dc2626")),
        str(cfg_select(plotting_config, "colors.translated", "#2563eb")),
    ]

    rows_to_plot = unique_projection_rows(ranked, top_k=top_k)
    if not rows_to_plot:
        return

    for row in rows_to_plot:
        candidate_dir = Path(str(row.get("out_dir", "")))
        fake_dir = candidate_dir / "fake"
        if not fake_dir.exists():
            continue

        fake_paths = sample_paths(
            discover_image_paths(fake_dir, prefer_parent_name=None),
            num_samples,
            seed,
        )
        stem = sanitize_filename(row_label(row))
        fake_emb = load_or_compute_embeddings(
            embedder,
            fake_paths,
            batch_size,
            cache_dir / f"{stem}_embed.npy",
        )

        for proj_name, (projector, sim_coords, real_coords) in projector_bundle.items():
            fake_coords = projector.transform(fake_emb)
            proj_dir = projections_dir / proj_name
            proj_dir.mkdir(parents=True, exist_ok=True)

            plot_projection_figure(
                coords=[sim_coords, real_coords, fake_coords],
                labels=labels,
                colors=colors,
                title=f"{proj_name.upper()} | {row_label(row)}",
                xlabel=f"{proj_name.upper()} 1",
                ylabel=f"{proj_name.upper()} 2",
                save_path=proj_dir / f"{stem}.pdf",
...
def plot_top_bars(ranked: list[dict[str, Any]], out_dir: Path, top_k: int):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "top_candidates_selection_score.pdf", dpi=180)
    plt.close(fig)
...
def plot_realism_vs_lpips(ranked: list[dict[str, Any]], out_dir: Path, top_k: int):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "realism_vs_lpips.pdf", dpi=180)
    plt.close(fig)
...
def plot_metric_correlations(candidates: list[dict[str, Any]], out_dir: Path):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "metric_correlations.pdf", dpi=180)
    plt.close(fig)
...
def plot_best_by_checkpoint(candidates: list[dict[str, Any]], out_dir: Path):
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "best_by_checkpoint.pdf", dpi=180)
    plt.close(fig)
...
def plot_heatmaps(candidates: list[dict[str, Any]], metrics: list[str], out_dir: Path):
    ...
    for metric in metrics:
        ...
        fig.savefig(out_dir / f"heatmap_{metric}.pdf", dpi=180, bbox_inches="tight")
        plt.close(fig)
...
def main():
    ...
    projection_config_path = Path("configs/cyclenet/eval/project_translated.yaml")
    make_projection_plots = True
                    ...
                    if make_projection_plots:
                        try:
                            plot_candidate_projections(
                                ...
                                cache_root=sweep_dir / "projection_cache",
                            )
                        except Exception as exc:
                            print(f"Skipping projection plots for {out_dir}: {type(exc).__name__}: {exc}")
